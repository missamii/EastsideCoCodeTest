...
var mongoUrl = 'mongodb://localhost:8080/social_db';
...
app.get('/', function(req, res){
  res.json({"description":"Posts like a API"});
});
...
/* delete */
app.delete('/username/:name', function(req, res) {
  ...
  MongoClient.connect(mongoUrl, function (err, db) {
    var favoriteCollection = db.collection('delete saved item');
    if (err) {
      console.log('Unable to connect to the mongoDB server. ERROR:', err);
    } else {
      // We are connected!
...
app.put('/:username', function(req, res) {
  ...
  MongoClient.connect(mongoUrl, function (err, db) {
    ...
    if (err) { //handles errors
      ...
      // We are connected!
      console.log('Updating by username... ');
